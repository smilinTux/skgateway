...
# Two true 64K slots: llama.cpp divides the total context across --parallel
# slots, so 131072 total preserves the qualified 65536 per-session window.
exec /mnt/comfyui/beellama.cpp/build/bin/llama-server \
  --model /mnt/comfyui/models/beellama/Ornith-1.5-9B-Q6_K.gguf \
  --alias ornith-1.5-9b \
  -ngl 99 --parallel 2 --ctx-size 131072 --batch-size 2048 \
  --reasoning on \

...
- `.100:8082` serves `ornith-1.5-9b` from
  ...
  **65536-token context**, qualified at 32K/48K/60K prompt depths; registry and gateway
  ...
  production server uses three slots with `--parallel 3 --ctx-size 196608`, preserving
  65536 tokens per slot. Three-way 32K and 48K concurrency is qualified; a fourth slot
  is prohibited on the 16GB card because it removes the measured VRAM safety margin.
...
- The direct `chiap08-qwen38` route and registry materialization `reg:qwen38`
  ...
  `tests/qwen-capacity-domain.test.mjs` before deployment. A configured domain
  is visible with zero active/queued counts before its first request. Pool
  slots are released only with the exact, single-use ticket returned by
  `acquire()`; string ids and copied/foreign tickets deliberately do nothing.
...
| `GET`  | `/queue` | Connection-pool depth and admission counters per capacity domain (or per backend when no domain is configured). |
...
| A Qwen id reports EOL despite a local declaration | Run the claim-aware tests and inspect the lifecycle record's provider attribution. A non-claiming provider 404/410 must not preempt a local claimer; separately verify `chiap08:11439` is reachable. |

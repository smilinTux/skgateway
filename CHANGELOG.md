...
- Normalize provider failure attribution, enforce durable circuit admission,
  and bound failover to three actual upstream attempts in independent capacity
...
- Treat bounded Z.ai `reasoning_content` as a valid completion signal without
  relaxing the fail-closed check for responses with no usable content.
...
- Codex capacity transitions now require accepted append-only audit evidence
  before persistent state changes, and a rejected probe-attempt event releases
  only its exact in-memory probe owner so recovery can retry safely.
